...
def _memory_safe_batch_processing(pipeline_list, fn_list, batch_size=25, restart_every=1): ##this did not work
    """
    Process pipelines in memory-safe batches using Dask futures.

    Parameters
    ----------
    pipeline_list : list
        List of PDAL pipelines to execute.
    fn_list : list
        List of filenames corresponding to the pipelines.
    batch_size : int, optional
        Number of tasks to process in each batch, by default 50.
    restart_every : int, optional
        Number of batches after which workers are restarted, by default 3.

    Returns
    -------
    list
        List of filenames generated by the pipelines.
    """
    # Set up Dask cluster
    #cluster = LocalCluster(threads_per_worker=1, n_workers=5, memory_limit='10GB')
    cluster = LocalCluster(threads_per_worker=1, n_workers=5)
    client = Client(cluster)

    # Split tasks into batches
    batches = [
        (pipeline_list[i:i + batch_size], fn_list[i:i + batch_size])
        for i in range(0, len(pipeline_list), batch_size)
    ]

    final_results = []

    for batch_idx, (batch_pipelines, batch_fns) in enumerate(batches):
        print(f"Processing batch {batch_idx + 1}/{len(batches)}")

        # Restart workers periodically
        if batch_idx > 0 and batch_idx % restart_every == 0:
            print("Restarting workers...")
            client.restart()

        # Submit tasks as futures
        futures = [
            client.submit(dsm_functions.execute_pdal_pipeline, pipeline, fn)
            for pipeline, fn in zip(batch_pipelines, batch_fns)
        ]
